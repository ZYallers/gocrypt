...
// 公钥加密或解密byte
func cryptPubKeyByte(pub *rsa.PublicKey, in []byte, encrypt bool) ([]byte, error) {
	k := (pub.N.BitLen() + 7) / 8
	if encrypt {
		k = k - 11
	}
	if len(in) <= k {
		if encrypt {
			...
			return decryptPubKey(pub, in)
		}
	} else {
		iv := make([]byte, k)
		out := bytes.NewBuffer(iv)
		if err := cryptPubKeyIO(pub, bytes.NewReader(in), out, encrypt); err != nil {
			return nil, err
		}
...
// 私钥加密或解密byte
func cryptPriKeyByte(pri *rsa.PrivateKey, in []byte, encrypt bool) ([]byte, error) {
	k := (pri.N.BitLen() + 7) / 8
	if encrypt {
		k = k - 11
	}
	if len(in) <= k {
		if encrypt {
			return encryptPriKey(rand.Reader, pri, in)
		} else {
		...
		iv := make([]byte, k)
		out := bytes.NewBuffer(iv)
		if err := cryptPriKeyIO(pri, bytes.NewReader(in), out, encrypt); err != nil {
			return nil, err
		}
		return ioutil.ReadAll(out)
	}
...
// 公钥加密或解密Reader
func cryptPubKeyIO(pub *rsa.PublicKey, in io.Reader, out io.Writer, encrypt bool) (err error) {
	k := (pub.N.BitLen() + 7) / 8
	if encrypt {
		k = k - 11
	}
...
// 私钥加密或解密Reader
func cryptPriKeyIO(pri *rsa.PrivateKey, r io.Reader, w io.Writer, encrypt bool) (err error) {
	k := (pri.N.BitLen() + 7) / 8
	if encrypt {
		k = k - 11
	}
	...
	for {
		...
		if encrypt {
			b, err = encryptPriKey(rand.Reader, pri, b)
		} else {
...
// 公钥解密
func decryptPubKey(pub *rsa.PublicKey, data []byte) ([]byte, error) {
	k := (pub.N.BitLen() + 7) / 8
	if k != len(data) {
...
// 私钥加密
func encryptPriKey(rand io.Reader, pri *rsa.PrivateKey, hashed []byte) ([]byte, error) {
	tLen := len(hashed)
	k := (pri.N.BitLen() + 7) / 8
	if k < tLen+11 {
		return nil, ErrDataLen
